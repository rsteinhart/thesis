...
for fcst in $(seq -f "%03g" 0 6 384); do
      wgrib2 $IN_DIR/merged_2019082700/cmc_merged.t00z.pgrb2f${fcst} -match ":(UGRD|VGRD):(5000.0|10000.0|30000.0|40000.0) Pa:" -grib $OUT_DIR/cmc_unq/cmc_unq.t00z.pgrb2f${fcst}
...
for fcst in $(seq -f "%03g" 0 6 384); do
    ...
    #   wgrib2 $IN_DIR/ncep_*.t00z.pgrb2f${fcst}_BC -match ":(UGRD|VGRD|HGT|TMP|RH):(20000.0|25000.0|50000.0|70000.0|85000.0|92500.0|100000.0) Pa:" -grib $OUT_DIR/common/common.t00z.pgrb2f${fcst}
      ...
      wgrib2 $IN_DIR/merged_2019082700/*.t00z.pgrb2f$fcst -match ":(WEASD|TCDC|PRES|PRMSL|PWAT|WEL):" -append -grib $OUT_DIR/common/common.t00z.pgrb2f${fcst}
